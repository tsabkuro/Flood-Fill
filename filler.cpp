...
template <template <class T> class OrderingStructure> animation filler::Fill(FillerConfig& config)
{
  ...
  */
  ...
  // 0 is not-processed : 1 is processed


  vector<vector<int>> visited;
  visited.resize(config.img.width());
  for(int i=0; i < config.img.width(); i++){
      visited[i].resize(config.img.height());
      for(int k=0; k < config.img.height(); k++){
          visited[i][k] = 0;
      }
  }
    ...
    config.seedpoint.color = config.picker->operator()(config.seedpoint);
    framecount++;
    while(!os.IsEmpty()) {

      PixelPoint currPixelPoint = os.Remove(); // get next pixel point and pop it off the list

    //   cout << currPixelPoint.x << endl;

      HSLAPixel* pixelAtcurrPixelPoint = config.img.getPixel(currPixelPoint.x, currPixelPoint.y);
      *pixelAtcurrPixelPoint = config.picker->operator()(currPixelPoint);

      if (currPixelPoint.x + 1 < config.img.width() && visited[currPixelPoint.x + 1][currPixelPoint.y] == 0 &&
      config.neighbourorder.GetReferenceColor().dist(*config.img.getPixel(currPixelPoint.x + 1, currPixelPoint.y)) <= config.tolerance) {
          config.neighbourorder.Insert(PixelPoint(currPixelPoint.x + 1, currPixelPoint.y));
          visited[currPixelPoint.x+1][currPixelPoint.y] = 1; // mark as visited
      }

      if (currPixelPoint.y + 1 < config.img.height() && visited[currPixelPoint.x][currPixelPoint.y + 1] == 0 &&
      config.neighbourorder.GetReferenceColor().dist(*config.img.getPixel(currPixelPoint.x, currPixelPoint.y+1)) <= config.tolerance) {
          config.neighbourorder.Insert(PixelPoint(currPixelPoint.x, currPixelPoint.y + 1));
          visited[currPixelPoint.x][currPixelPoint.y+1] = 1; // mark as visited
      }

      if ((int)currPixelPoint.x - 1 >= 0 && visited[(int)currPixelPoint.x - 1][currPixelPoint.y] == 0 &&
      config.neighbourorder.GetReferenceColor().dist(*config.img.getPixel(currPixelPoint.x - 1, currPixelPoint.y)) <= config.tolerance) {
          config.neighbourorder.Insert(PixelPoint(currPixelPoint.x - 1, currPixelPoint.y));
          visited[currPixelPoint.x-1][currPixelPoint.y] = 1; // mark as visited
      }

      if ((int)currPixelPoint.y - 1 >= 0 && visited[currPixelPoint.x][(int)currPixelPoint.y - 1] == 0 &&
      config.neighbourorder.GetReferenceColor().dist(*config.img.getPixel(currPixelPoint.x, currPixelPoint.y-1)) <= config.tolerance) {
          config.neighbourorder.Insert(PixelPoint(currPixelPoint.x, currPixelPoint.y - 1));
          visited[currPixelPoint.x][currPixelPoint.y-1] = 1; // mark as visited
      }
